...
import { authOptions } from "../../lib/auth" // ✅ Sèl chemen kòrèk
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (userQuota <= 0) return res.status(403).json({ success: false, error: "You have used all your free checks" })

  if (req.method !== "POST") return res.status(405).json({ success: false, error: "Method Not Allowed" })

  const { imei } = req.body
  if (!imei || typeof imei !== "string") return res.status(400).json({ success: false, error: "Missing or invalid IMEI" })
