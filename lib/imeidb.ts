// lib/imeidb.ts
export interface ImeidbOk { success: true; data: unknown; }
export interface ImeidbErr { success: false; error: string; }
...
export async function imeidbCheck(imei: string): Promise<ImeidbResult> {
  ...
  if (!base || !key) return { success: false, error: "Missing IMEIDB_API_BASE or IMEIDB_API_KEY" };
  if (!imei || imei.length < 10) return { success: false, error: "Invalid IMEI" };
  ...
  try {
    ...
    if (!json) return { success: false, error: "IMEIDB: invalid JSON" };
